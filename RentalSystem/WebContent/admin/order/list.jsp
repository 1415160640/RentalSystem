								...
								<c:forEach items="${page.list}" var="c" varStatus="status">
										<tr onmouseover="this.style.backgroundColor = 'white'"
											...
											<td style="CURSOR: hand; HEIGHT: 25px" align="center"
												...
												<c:if test="${c.state ==1 }">
													<a href="/RentalSystem/AdminOrderServlet?method=OutMoney&itemid=${c.itemid}" style="color:blue; font-size:1.5em;">转出租金</a>
												</c:if>
												<c:if test="${c.state ==2 }">
													<a href="/RentalSystem/AdminOrderServlet?method=OutMoney1&itemid=${c.itemid}" style="color:red; font-size:1.5em;">退定金</a>
												</c:if>
												<c:if test="${c.state ==3 }">
													等待退房
												</c:if>
